...
- 群組或多人聊天室情境中，新增記帳統一使用分攤格式；分攤成員預設只勾選目前開啟 LIFF 的使用者。
- 群組或多人聊天室情境中，`消費項目 > 個人消費` 代表共享帳本中分攤成員僅 1 人的項目，並以 `消費歸屬` 顯示該項目歸屬者；`團體消費` 代表分攤成員 2 人以上的項目。
- `記帳者` 僅代表建立或修改紀錄的人；`付款人` 代表實際先付錢的人；`消費歸屬` 由單一分攤成員決定。
- 團體統計支援付款、應付、差額、計算過程、每人淨額與誰欠誰建議；不同幣別分開計算，不做匯率換算。
...
目前 LIFF 前端不提供待結清 / 已結清操作；此表保留為既有 migration 與未來可能恢復結清狀態時使用。
...
- 依幣別分組的計算過程：平均分攤顯示 `金額 ÷ 分攤成員人數`，指定金額顯示每位成員的個別分攤。
- 每人淨額：顯示每位成員 `已支付 - 應分攤 = 應收 / 應付`。
- 結算清單：顯示同幣別淨額簡化後的建議付款方式，不提供待結清 / 已結清操作。
...
- `刪除` 只代表該成員不再參與目前帳本；統計、結算、消費列表與幣別明細會排除 inactive 成員。
- 目前不提供成員 ID 合併功能。若成員建錯或旅伴名稱需替換，操作方式是刪除舊成員、加入新成員，編輯受影響消費並重新勾選分攤成員。
- 在 `指定金額` 編輯情境刪除成員時，該成員在目前表單中的指定金額會一併移除，畫面會提示目前尚差或超出的金額，由使用者手動填到新成員。
- 若舊消費紀錄仍保存已刪除成員的 `split_members` 快照，編輯該筆時會顯示為已刪除成員；使用者需改選 active 成員並手動調整分攤金額。

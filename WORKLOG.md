...
- 補充和歌山 Day 3 高野山午餐資訊：
  - `精進料理` 新增料理介紹連結。
  - 新增 `午餐建議`，列出角濱ごまとうふ総本舗 大門店、茶房みやざき、寺カフェ成慶院、中央食堂 SANBOU、SHOJIN Dining 桐寶珠與 Tsukumo 的 Google Maps。
...
- 重新收斂 LIFF 記帳語意與 DOM：
  - `我要記帳` 移除個人/多人記帳判斷與 `add-scope-card`，固定保留團體分帳欄位結構。
  - `付款人` 統一改為 `付款 / 代墊人`。
  - 金額 placeholder 改為 `請輸入消費金額`；備註 placeholder 改為 `請輸入備註內容`。
  - `付款 / 代墊人` 與 `分帳方式` 拉出 `split-panel`，作為 full-row 欄位。
  - `分攤成員` 依平均分攤 / 指定金額調整左右欄與刪除按鈕寬度，避免 RWD 跑版。
- 重整 `消費項目`：
  - `個人消費` 改為以消費歸屬成員分組，顯示成員頭貼與 ID，右側 toggle icon 統一為箭頭樣式。
  - 移除個人消費內的 `依日期` / `依幣別` segmented，改在成員區塊內顯示目前消費統計文字。
  - 消費項目標題加入消費日期 `mm/dd`。
  - `團體消費` 移除排序 segmented，改依幣別分成 `台幣` / `日幣` 可展開區塊。
  - 團體消費項目 DOM 調整為左欄標題與 meta、右欄金額；補多項 RWD CSS 微調。
- 重整 `統計`：
  - 移除統計頁的個人/多人 segmented，固定顯示分帳統計與結算清單。
  - 純個人消費：分攤成員僅 1 人且 `付款 / 代墊人` 等於該成員時，不納入分帳統計與結算。
  - 個人代墊語意改以 `個人消費` 顯示；單人分攤但付款人不同時仍納入結算計算。
  - `計算過程` 中 `split-calc-item-title` 統一使用 `分類 / 備註` 格式。
  - `.split-currency-title` 調整為 `font-size: 1.25rem`、`margin-top: 12px`。
- 修正編輯既有消費時跳出 `找不到這筆記帳，可能已經刪除。` 的問題，讓修改付款 / 代墊人、分攤成員與其他欄位可正常保存。
- LINE bot 與旅程入口調整：
  - 旅行工具 Flex 按鈕 `完整行程` 改為 `旅遊行程`。
  - 旅程設定頁說明文字同步改為 `旅遊行程`。
  - LIFF 記帳頁 `<h1>` 與 `document.title` 會依旅程名稱顯示，例如 `土耳其旅行記帳本`、`和歌山旅行記帳本`。
  - 修正和歌山標題一度被抓成 `2026旅行記帳本` 的問題；現在 `2026 和歌山女子之旅` 與 `2026-06-wakayama` 都會推導為 `和歌山`。
...
- `node --check webhook/cloudflare-worker/src/index.js` 通過。
- `npm run build:data` 通過。
- 多次 `npm run deploy` 成功。
- 最新已部署 Worker version ID：`1a9e2445-70e3-455b-9e59-9e37a743ae5f`。

### 後續保留

1. 使用 LINE 手機實測最新 `我要記帳`、`消費項目`、`統計` 在和歌山群組下的標題與顯示是否都符合預期。
2. 繼續保留第三階段：LINE id token 強驗證、離開群組後撤權、行程提醒 workflow restore/save 復測。
3. repo rename 仍暫緩，之後確認時再改為 `azuma-papago`。
...
- 修正群組情境中的 `我的消費` 帳本隔離：
  ...
  - 新增一次性相容搬移：明確鎖定 `dev-sandbox` 的 LINE 群組 `C87ffe42ff346bc573d7eb45a9cbec853`，使用者從該群組查看 `我的消費` 時，會把該使用者舊的 `personal:<userId>`、空 ledger，或先前誤搬到其他群組 personal ledger 的舊個人消費搬到該群組 personal ledger。
- 修正手動成員與 LINE 成員重複造成的分帳統計殘留：
  - 分帳成員列新增 `ID合併` icon，與刪除 icon 分開。
  - `ID合併` 需由使用者明確選擇目標成員；合併後會把來源成員的歷史付款人與 `split_members` ID 改成目標成員 ID，金額不重新分攤。
  - 合併後來源成員會改為 inactive，相關結算狀態會作廢，讓統計重新產生。
  - `分帳統計`、`結算清單`、`相關結算依據` 會排除 inactive 成員，避免已刪除的手動成員如 `皮皮` 繼續殘留。
  - 刪除成員前會提示：若此成員其實是另一位 LINE 成員，應改用 `ID合併`。
- 依後續實測回饋補強刪除與合併語意：
  - `皮皮`、`皮皮2` 等不同時期手動新增的 ID，在使用者明確合併前一律視為不同成員。
  - `消費項目` 列表與幣別明細改為排除 inactive 分攤成員，避免 3 人分帳畫面殘留 4 個 ID。
  - 編輯舊消費時，已刪除但仍存在於該筆 `split_members` 快照的歷史成員會顯示為 `已刪除`，並保留 `ID合併` 入口。
  - 已刪除成員仍可作為 `ID合併` 來源；合併目標只允許 active 成員。
  - `指定金額` 合併會把來源成員金額移到目標成員欄位，若目標已有金額則加總，不重新分攤。
- 依 #64 指定金額合併實測回饋再修正：
  - `ID合併` icon 改為 lucide `git-merge`，成員列 icon button 調整為圓形樣式。
  - 編輯既有消費時，按 `ID合併` 只會在目前表單預覽替換結果，不會立即呼叫後端合併 API。
  - 來源成員的指定金額會立刻顯示在合併目標的 input 中，例如 `皮皮3` 的 800 會直接帶到 `皮拉提2`。
  - 按 `儲存修改` 才會更新該筆消費；按 `取消` 不會異動原本 #64 資料。

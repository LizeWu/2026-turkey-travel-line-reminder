...
1. 在 `統計 > 團體消費` 新增 `幣別淨額` 區塊，與 `分帳統計`、`結算` 並列。
2. `幣別淨額` 用來呈現同幣別加總後的淨額：同一幣別內 `已支付 - 應分攤`，不同幣別不互相抵銷。
3. 繼續在 LINE 群組用 `dev-sandbox` 實測指定金額分攤、逐筆結算、待結清/已結清切換與新增的幣別淨額。
4. 第三階段再處理 LINE id token 強驗證、離開群組後撤權與行程提醒 workflow 復測。
